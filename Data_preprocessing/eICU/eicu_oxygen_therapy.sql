...
--Change the final output table
WITH respchart AS (
	SELECT *
	FROM `oxgenator.eicu.respiratorycharting`
)

, nursechart AS (
	SELECT *
	FROM `oxgenator.eicu.nursecharting`
)

, pat AS (
	SELECT *
	FROM `oxgenator.eicu.patient`
)
...
SELECT *, ROW_NUMBER() OVER (partition by icustay_id order by charttime) AS RN
